...
public class BoardApiController {
    ...
    public Map<String, Object> makeApi(List<Board> boards) {
        ...
        for (Board board : boards) {
            ...
            item.put("description", board.getTexts());
    ...
    public Map<String,Object> searchTitle(@RequestBody String title) {
        //검색하고 싶은 제목을 인자에 넣음
        Map<String, Object> content;
        try{
            List<Board> boards =  boardService.searchBoard(title);
            content =  makeApi(boards);
        }catch(Exception e) {
            throw new CustomException(FAILED_TO_GET_RECIPE_LIST);
        }
...
//        formData= formData.substring(1,formData.length()-2);
    ...
    @PostMapping (value = "/board/mypage-like")
    public Map<String,Object> myPageLike (@RequestBody String nickName){
        Map<String,Object> content;
        List<LikeCheck> LikeChecks = likeCheckService.getMyLikeTitle(nickName);
        System.out.println("좋아요 누른 닉네임: "+nickName);
        ...
        for(LikeCheck likeCheck:LikeChecks){
            ...
            boards.add(boardService.getUserBoard(boardId));
        }
        content = makeApi(boards);
        return content;
    }
